...
mv_1        =  c(1576.7750, 1366.6431, 24812.8720, 927.4684)  # long-run estimate
...
load("sim_model1_n200.RData")             # long term simulated network
...
if(length(degen) > 0 ){
  degen_ergm_tapered = foreach(i = 1:length(degen)) %dorng% {
    skip_to_next <- FALSE
    formula <- g_sim[[degen[i]]] ~ edges + nodematch("x") + kstar(2) + triangles
    ergm_sim = 
      tryCatch(withTimeout(
        ergm.tapered(formula, eval.loglik=FALSE,
                     control=control.ergm.tapered(init = init_params[degen[i],]
                                                  # , MCMC.samplesize=10000
                     )),
        timeout = 5*60, onTimeout = "error"),
        error = function(e) {skip_to_next <<- NULL})
  ...
  {
    ergm_sim_estim[degen[i],] <- degen_ergm_tapered[[i]]$coefficients
...
ergm_result = apply(ergm_sim_estim, 2, mean)
ergm_result
