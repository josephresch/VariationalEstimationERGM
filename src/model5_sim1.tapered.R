...
summary(fit$newnetwork ~ edges + nodematch("x") + kstar(2) + triangles)
theta <- coef(fit)
theta
fit$tapering.coefficients
fit$tapering.coef

# Simulated networks 'g_sim' used for both ERGM and MFERGM
g_sim <- simulate_ergm.tapered(fit$newnetwork ~ edges + nodematch("x") + kstar(2) + triangles, 
                  nsim = 10*nsims, tapering.centers=fit$tapering.centers, tau=fit$tapering.coefficients,
                  coef = coef(fit),
...
cbind(mv_1, mv_i, mv_s)

mv_s <- apply(g_sim_stats,2,mean)
names(mv_s) <- names(fit$tapering.coefficients)
cbind(0.25/mv_s,fit$tapering.coefficients)

g_sim <- simulate_ergm.tapered(fit$newnetwork ~ edges + nodematch("x") + kstar(2) + triangles, 
                  nsim = 10*nsims, tapering.centers=mv_i, tau=0.25/mv_i,
                  coef = coef(fit),
		  control=control.simulate.formula(MCMC.burnin=100000, MCMC.interval=10000)
                              )
...
cbind(mv_1, mv_i, apply(g_sim_stats,2,mean))
...
for (i in 1:nsims)
{
  init_params[i,] = ergm.tapered(g_sim[[i]] ~ edges + nodematch("x") + kstar(2) + triangles, 
...
ergm_sim_estim
# compare mean-values
ergm_mv_est = foreach(i = 1:nsims, .combine = rbind) %dopar% {
...
# compare mean-values
ergm_mv_est_tapered = foreach(i = 1:nsims, .combine = rbind) %dopar% {
...
# compare mean values
mf_mv_est = foreach(i = 1:nsims, .combine = rbind) %dopar% {
 simulate_ergm.tapered(g_sim[[i]] ~ edges+nodematch("x")+ kstar(2) + triangles,
               nsim = 10,
               control=control.simulate.formula(MCMC.burnin=100000, MCMC.interval=10000),
               coef =  mfergm_estim[i,],    
...
# compare mean values
MPLE_mv_est = foreach(i = 1:nsims, .combine = rbind) %dopar% {
 simulate_ergm.tapered(g_sim[[i]] ~ edges+nodematch("x")+ kstar(2) + triangles,
               nsim = 10,
               coef = init_params[i,],    
               control=control.simulate.formula(MCMC.burnin=100000, MCMC.interval=10000),
