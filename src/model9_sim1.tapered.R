...
theta <- c(-4.126381258, 1.987910555, -0.003310214, 0.326106864)
theta <- c(-4.091060769, 2.004221372, -0.005750068, 0.329363331)
...
a = foreach(i = 1:10) %dorng% {
 simulate_ergm.tapered(sim ~ edges + nodematch("x") + kstar(2) + triangles, 
                  nsim = nsims/10, tapering.centers=mv_1, tau=0.25/mv_1,
                  coef = theta,
  ...
  ergm_sim = 
    tryCatch(withTimeout(
                          ergm.tapered(formula, eval.loglik=FALSE,
                               control=control.ergm.tapered(init = init_params[i,], MCMC.samplesize=10000)),
